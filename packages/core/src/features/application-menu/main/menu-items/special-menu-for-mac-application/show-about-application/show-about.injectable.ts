import { loggerInjectionToken } from "@kubesightapp/logger/dist";
...
const showAboutInjectable = getInjectable({
  ...
  instantiate: (di) => {
    ...
    return async () => {
      ...
      try {
        const latestVersion = await getLatestVersion("@kubesightapp/core");
      ...
      if (newVersion && returnValue.response === 1) {
        await openLinkInBrowser(`https://github.com/GlobalArtInc/k8sight/releases/tag/v${newVersion}`);
      }
    };
  },
});
