...
import type { ElectronWindow } from "./create-k8sight-window.injectable";
...
const createElectronWindowInjectable = getInjectable({
  ...
  instantiate: (di): CreateElectronWindow => {
    ...
    return (configuration) => {
      ...
      if (isLinux) {
        const iconFileName = [
          getAbsolutePath(k8sightResourcesDir, `../${applicationInformation.name}.png`),
      ...
      browserWindow
        ...
        .setWindowOpenHandler((details) => {
          if (!details.url.includes(".renderer.k8sight.app:")) {
