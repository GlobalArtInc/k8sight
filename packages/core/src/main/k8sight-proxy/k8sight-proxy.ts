...
export type K8sightProxyApiRequest = (args: ProxyApiRequestArgs) => void | Promise<void>;

interface Dependencies {
  getClusterForRequest: GetClusterForRequest;
  shellApiRequest: K8sightProxyApiRequest;
  kubeApiUpgradeRequest: K8sightProxyApiRequest;
  emitAppEvent: EmitAppEvent;
  getKubeAuthProxyServer: (cluster: Cluster) => KubeAuthProxyServer;
  readonly router: Router;
  readonly proxy: httpProxy;
  readonly k8sightProxyPort: { set: (portNumber: number) => void };
...
export class K8sightProxy {
    ...
    this.proxyServer.on("upgrade", (req: ServerIncomingMessage, socket: net.Socket, head: Buffer) => {
      ...
      if (!cluster) {
        ...
        (async () => reqHandler({ req, socket, head, cluster }))().catch((error) =>
          this.dependencies.logger.error("[K8SIGHT-PROXY]: failed to handle proxy upgrade", error),
        );
      }
    });
  }
    ...
    return new Promise<number>((resolve, reject) => {
      ...
      this.proxyServer
        .once("listening", () => {
          ...
          this.dependencies.k8sightProxyPort.set(port);

          this.dependencies.logger.info(`[K8SIGHT-PROXY]: Proxy server has started at ${address}:${port}`);

          this.proxyServer.on("error", (error) => {
            this.dependencies.logger.info(`[K8SIGHT-PROXY]: Subsequent error: ${error}`);
          });

          this.dependencies.emitAppEvent({ name: "k8sight-proxy", action: "listen", params: { port } });
          resolve(port);
        })
        .once("error", (error) => {
          this.dependencies.logger.info(`[K8SIGHT-PROXY]: Proxy server failed to start: ${error}`);
          reject(error);
        });
    });
  }
  ...
  async listen(): Promise<void> {
    ...
    while (true) {
      ...
      this.dependencies.logger.warn(
        `[K8SIGHT-PROXY]: Proxy server has with port known to be considered unsafe to connect to by chrome, restarting...`,
      );
      ...
      if (seenPorts.has(port)) {
         ...
         * port.
         */
        throw new Error("Failed to start K8Sight Proxy due to seeing too many unsafe ports. Please restart K8Sight.");
      } else {
        seenPorts.add(port);
      }
    }
  ...
  close() {
    ...
    this.closed = true;
    this.dependencies.logger.info("[K8SIGHT-PROXY]: Closing server");
  ...
  protected configureProxy(proxy: httpProxy): httpProxy {
    ...
    proxy.on("error", (error, req, res, target) => {
      ...
      this.dependencies.logger.error(`[K8SIGHT-PROXY]: http proxy errored for cluster: ${error}`, { url: req.url });
          ...
          if (retryCount < 20) {
            ...
            setTimeout(() => {
              ...
              this.handleRequest(req as ServerIncomingMessage, res).catch((error) =>
                this.dependencies.logger.error(`[K8SIGHT-PROXY]: failed to handle request on proxy error: ${error}`),
              );
            }, timeoutMs);
          }
        }
      ...
      try {
        ...
        this.dependencies.logger.error(`[K8SIGHT-PROXY]: Failed to write headers: `, e);
      }
    });
