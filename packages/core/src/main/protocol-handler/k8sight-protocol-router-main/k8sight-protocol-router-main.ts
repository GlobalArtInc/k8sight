import { disposer, noop } from "@kubesightapp/utilities";
...
import type { K8sightProtocolRouterDependencies, RouteAttempt } from "../../../common/protocol-handler";
 ...
 * @returns `true` if it should be routed internally to K8sight, `false` if to an extension
...
export interface K8sightProtocolRouterMainDependencies extends K8sightProtocolRouterDependencies {
  showApplicationWindow: () => Promise<void>;
  broadcastMessage: BroadcastMessage;
}

export class K8sightProtocolRouterMain extends proto.K8sightProtocolRouter {
  ...
  constructor(protected readonly dependencies: K8sightProtocolRouterMainDependencies) {
    super(dependencies);
  }
  ...
  public async route(rawUrl: string) {
    try {
      ...
      if (url.protocol.toLowerCase() !== "k8sight:") {
      ...
      this.dependencies.logger.info(`${proto.K8sightProtocolRouter.LoggingPrefix}: routing ${url.toString()}`);
      ...
      if (error instanceof proto.RoutingError) {
        this.dependencies.logger.error(`${proto.K8sightProtocolRouter.LoggingPrefix}: ${error}`, { url: error.url });
      } else {
        this.dependencies.logger.error(`${proto.K8sightProtocolRouter.LoggingPrefix}: ${error}`, { rawUrl });
      }
    }
  }
