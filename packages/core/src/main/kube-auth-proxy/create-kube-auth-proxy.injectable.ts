...
import k8SightK8sProxyPathInjectable from "./k8sight-k8s-proxy-path.injectable";
...
const createKubeAuthProxyInjectable = getInjectable({
  ...
  instantiate: (di, cluster): CreateKubeAuthProxy => {
    const k8SightK8sProxyPath = di.inject(k8SightK8sProxyPathInjectable);
    ...
    return (env) => {
      ...
      const run = async (): Promise<void> => {
        ...
        proxyProcess = spawn(k8SightK8sProxyPath, [], {
          env: {
            ...env,
