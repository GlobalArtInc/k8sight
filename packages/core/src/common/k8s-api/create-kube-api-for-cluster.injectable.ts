import { KubeApi } from "@kubesightapp/kube-api";
...
import type { KubeJsonApiDataFor, KubeObject, KubeObjectConstructor } from "@kubesightapp/kube-object";
  ...
  instantiate: (di): CreateKubeApiForCluster => {
    ...
    return (
      ...
      const request = createKubeJsonApi(
        ...
        {
          headers: {
            Host: `${cluster.metadata.uid}.renderer.k8sight.app:${new URL(apiBase.config.serverAddress).port}`,
          },
        },
      );
