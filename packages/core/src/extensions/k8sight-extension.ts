...
import type { InstalledExtension, K8sightExtensionId, K8sightExtensionManifest } from "@kubesightapp/legacy-extensions";
...
export interface K8sightExtensionDependencies {
  readonly logger: Logger;
  ensureHashedDirectoryForExtension: EnsureHashedDirectoryForExtension;
}

export class K8sightExtension {
   ...
   * @ignore
   */
  protected readonly dependencies: K8sightExtensionDependencies;
  ...
  constructor(deps: K8sightExtensionDependencies, { id, manifest, manifestPath, isBundled }: InstalledExtension) {
    this.dependencies = deps;
    this.id = id;
    this.manifest = manifest as K8sightExtensionManifest;
