import { disposer } from "@kubesightapp/utilities";
...
export const appPaths: Partial<Record<NodeJS.Platform, string>> = {
  win32: "./dist/win-unpacked/K8Sight.exe",
  linux: `./dist/linux${process.arch === "arm64" ? "-arm64" : ""}-unpacked/k8sight`,
  darwin: `./dist/mac${process.arch === "arm64" ? "-arm64" : ""}/K8Sight.app/Contents/MacOS/K8Sight`,
};
...
async function getMainWindow(app: ElectronApplication, timeout = 50_000): Promise<Page> {
  return new Promise((resolve, reject) => {
    ...
    const onWindow = (page: Page) => {
      ...
      if (page.url().startsWith("https://renderer.k8sight.app")) {
        cleanup();
    ...
    const timeoutId = setTimeout(() => {
      cleanup();
      console.log(stdoutBuf);
      reject(new Error(`K8Sight did not open the main window within ${timeout}ms`));
    }, timeout);
...
async function attemptStart() {
  const K8SIGHT_INTEGRATION_TESTING_DIR = path.join(os.tmpdir(), "k8sight-integration-testing", uuid.v4());
  process.env.K8SIGHT_INTEGRATION_TESTING_DIR = K8SIGHT_INTEGRATION_TESTING_DIR;
  ...
  await remove(K8SIGHT_INTEGRATION_TESTING_DIR);
  // We need original .kube/config with kind context
  const testDir = path.join(K8SIGHT_INTEGRATION_TESTING_DIR, "home", ".k8sight", "extensions");
  ...
  const app = await electron.launch({
    ...
    bypassCSP: true,
    env: {
      K8SIGHT_INTEGRATION_TESTING_DIR,
      LOG_LEVEL: "debug",
      ...process.env,
    },
    timeout: 100_000,
  ...
  try {
    ...
    return {
      ...
      cleanup: async () => {
        try {
          await app.close();
          await withTimeout(remove(K8SIGHT_INTEGRATION_TESTING_DIR), 15_000);
        } catch (_e) {
          // no-op
        }
      },
