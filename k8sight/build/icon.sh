...
## Windows
convert -background none icon.svg -density 400 -define icon:auto-resize=256,16,20,24,32,40,48,60,64,72,80,96 -verbose icon.ico

## Linux
for i in 16 22 24 32 36 48 64 72 96 128 192 256 512; do
  border=$(("${i}000" * 38 / 1000000))
  size=$((i - 2 * border))
  convert -background none icon.svg -density 400 -resize "${size}x${size}" -bordercolor transparent -border "${border}" -verbose "icons/${i}x${i}.png"
done
