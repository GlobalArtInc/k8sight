import corePackageJson from "@kubesightapp/core/package.json";
...
const renderer: webpack.Configuration = {
  ...
  entry: {
    k8sight: path.resolve(rendererDir, "index.ts"),
  },
  output: {
  ...
  plugins: [
    ...
    new CopyPlugin({
      patterns: [
        {
          from: path.resolve(
            path.dirname(require.resolve("@kubesightapp/core/package.json")),
